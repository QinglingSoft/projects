...
<c:set target="${articlePage}" property="articleType" value="NEWS"/>
...
<script type="text/javascript">
	$(function(){
		$("table.list thead").addClass("ui-widget-header");
	});
</script>
</head>
<body>
<center>
<list:morePage pageHelper="${articlePage}" label="新闻" functionUrl="newNews.jsp?type=NEWS" functionLabel="撰写">
		...
		<td>
			<c:choose>
				<c:when test="${entry.published}">
					<c:url var="unpublishUrl" value="unpublishNews.action">
						<c:param name="articleId" value="${entry.id}" />
					</c:url>
					<a href="${unpublishUrl}">撤稿</a>
				</c:when>
				<c:otherwise>
					<c:url var="publishUrl" value="publishNews.action">
						<c:param name="articleId" value="${entry.id}" />
					</c:url>
					<a href="${publishUrl}">发布</a>
					<c:url var="editUrl" value="editNews.jsp">
						<c:param name="articleId" value="${entry.id}" />
					</c:url>
					<a href="${editUrl}">编辑</a>
					<c:url var="deleteUrl" value="deleteNews.action">
						<c:param name="articleId" value="${entry.id}" />
					</c:url>
